...
## Recent Updates
- **Nov 28, 2025**: نظام حماية الملفات الجذري - FileRetentionManager:
  - **المشكلة**: الملفات كانت تختفي من النظام بسبب أكواد الحذف المتعددة
  - **الحل الجذري**: 
    - ✅ إنشاء `services/file_retention.py` - نظام مركزي لحماية الملفات
    - ✅ إزالة جميع أكواد `os.remove()` و `os.unlink()` من الملفات الإنتاجية
    - ✅ تحويل جميع عمليات "الحذف" إلى حذف المرجع من DB فقط
    - ✅ الملفات الفعلية تبقى محفوظة للأمان
  - **الملفات المُصلحة**:
    - vehicles.py: 11 نقطة حذف → محمية
    - employee_requests.py: 3 نقاط حذف → محمية
    - api_employee_requests.py: 6 نقاط حذف → محمية
    - external_safety.py: 3 نقاط حذف → محمية
    - mobile.py: 1 نقطة حذف → محمية
    - employees.py: 8 نقاط حذف → محمية
    - documents.py, properties.py, attendance.py: محمية
  - **السياسة الجديدة**:
    - 💾 جميع الملفات المرفوعة تبقى دائماً
    - 💾 عند "الحذف" يتم إزالة المرجع من قاعدة البيانات فقط
    - 💾 الحذف الفعلي يتطلب تدخل يدوي
    - 💾 جميع التغييرات موسومة بـ 💾 للتتبع
  - **الاستثناءات المقبولة**:
    - ملفات التصدير المؤقتة (operations.py, sim_management.py)
    - ملفات Google Drive المؤقتة
    - ملفات الخرائط المؤقتة
